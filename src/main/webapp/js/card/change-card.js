$(function (changecard) {
    ...
    changecard.initTreeView = function () {
        $.ajax({
            type: 'get',
            data: {company_code: "1"},
            dataType: "json",
    ...
    changecard.initButton = function () {
        //查询
        ...
        $("#btn_change").click(function () {
            ...
            $("#card_code").val(selected[0].card_code);
            $("#staff_code").val(selected[0].staff_code);
            $("#staff_name").val(selected[0].staff_name);
            $("#card_deposit").val(selected[0].card_deposit);
            $("#card_costing").val(selected[0].card_costing);
            $("#card_ineffectived_ts").val(selected[0].card_ineffectived_ts);
        ...
        $("#okBtn").click(function(){
            ...
            var sendData = {
                ...
                card_deposit:$("#card_deposit").val()*100,
                card_cost:$("#card_costing").val()*100,
                cardcode_new:$("#card_code_new").val(),
            ...
            $.ajax({
                ...
                dataType: "json",
                url: getUrl() + '/Api/card/change',
                contentType: "application/json;charset=UTF-8",
                success: function (data) {
                    if (data.success) {
                        Notify('补卡成功', 'top-right', '5000', 'primary', 'fa-times-circle', true);
                    } else {
                        Notify('补卡失败', 'top-right', '5000', 'danger', 'fa-times-circle', true);
                    }
                },
                error: function (data) {
                     Notify('补卡失败', 'top-right', '5000', 'danger', 'fa-times-circle', true);
    ...
    changecard.refresh = function refresh() {
        var opt = {
    ...
    //1.初始化左侧树
    changecard.initTreeView();

    //2.初始化Table
    changecard.initTable();

    //3.初始化Button的点击事件
    changecard.initButton();
